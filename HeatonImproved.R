...
features <- vroom("./features.csv.zip")
  ...
  for(dept in unique(store_test$Dept)){
    ...
    if(nrow(dept_train)==0){
      ...
      my_recipe <- recipe(Weekly_Sales ~ ., data = dept_train) %>%
        ...
        step_mutate(
          Holiday = case_when(
            IsHoliday == TRUE & Date_month == "Feb" ~ "Super Bowl",
            IsHoliday == TRUE & Date_month == "Sep" ~ "Labor Day",
            IsHoliday == TRUE & Date_month == "Nov" ~ "Thanksgiving",
            IsHoliday == TRUE & Date_month == "Dec" ~ "Christmas",
            TRUE ~ "None")) %>%
...
vroom_write(x=all_preds, 
            file=paste0("./Predictions_newholiday.csv"), delim=",")
